...
const AnnouncementCard = ({ announcement, setAnnouncements }: Props) => {
  ...
  const handleDelete = async () => {
    ...
    const URL = `${ORG_URL}/${hackathon.organizationID}/hackathons/${hackathon.id}/announcements/${announcement.id}`;
  ...
  return (
    ...
    >
      ...
      <Link
        href={`${FRONTEND_URL}/explore/organisation/${announcement.organization?.user.username}`}
        target="_blank"
      ...
      <div className="w-[calc(100%-32px)] flex flex-col gap-2">
        <div className="w-full h-fit flex justify-between">
          ...
          >
            ...
            <div className="text-xs max-md:text-xxs font-normal text-gray-500">@{announcement.organization?.user.username}</div>{' '}
          </Link>
