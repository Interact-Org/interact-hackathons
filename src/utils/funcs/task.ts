...
export const getTaskPriorityColor = (task: Task | SubTask) =>
  task.priority == 'high' ? danger : task.priority == 'medium' ? warn : success;

export const getTaskDeadlineColor = (task: Task | SubTask) =>
  moment(task.deadline).isBefore(moment())
    ? danger
    : moment(task.deadline).subtract(1, 'day').isBefore(moment())
    ? warn
    : success;
