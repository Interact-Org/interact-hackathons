...
const EditTask = ({ setShow, org, organization = initialOrganization, project = initialProject, task, setTasks }: Props) => {
  ...
  const removeUser = async (userID: string, toaster: Id) => {
    const URL = org ? `${ORG_URL}/${currentOrgID}/tasks/users/${task.id}/${userID}` : `${TASK_URL}/users/${task.id}/${userID}`;
  ...
  return (
    <>
      ...
      <div onClick={() => setShow(false)} className="bg-backdrop w-screen h-screen fixed top-0 left-0 animate-fade_third z-20 max-lg:z-40"></div>
    </>
  );
};
